...
def aplicar_volumen_efectos():
    vol = 0.0 if sfx_muted else sfx_volume
    if hit_sound:
        hit_sound.set_volume(vol)
    for s in horn_sounds:
        s.set_volume(vol)
...
boton_nivel3_rect = None
victoria_bg = None
victoria_bg_cargada = False
gameover_bg = None
gameover_bg_cargada = False
...
def reiniciar_partida():
    ...
    global lista_obstaculos, scroll_x, game_over, tiempo_game_over, tiempo_nivel_completado, nivel_completado
...
def ejecutar_opcion_menu_pausa(opcion):
    global music_muted, sfx_muted, juego_activo
    if opcion == 0:
        cerrar_menu_pausa()
    elif opcion == 1:
...
# --- PANTALLA DE DIÁLOGO ANTES DEL NIVEL ---
dialogo_img = None
try:
    dialogo_path = os.path.join(BASE_DIR, 'imagenes', 'dialogo2.png')
    if os.path.exists(dialogo_path):
        dialogo_img = pygame.image.load(dialogo_path).convert()
except Exception as e:
    print(f"Error cargando dialogo2.png: {e}")

fuente_parpadeo = pygame.font.Font(None, 42)
dialogo_activo = True
tiempo_parpadeo_inicio = pygame.time.get_ticks()

while dialogo_activo:
    reloj.tick(60)
    ...
    if dialogo_img:
        VENTANA.blit(pygame.transform.scale(dialogo_img, (ANCHO, ALTO)), (0, 0))
    else:
        VENTANA.fill((0, 0, 0))

    # Texto parpadeante cada 500ms
    if (pygame.time.get_ticks() - tiempo_parpadeo_inicio) % 1000 < 500:
        texto_enter = fuente_parpadeo.render("Pulsa ENTER para comenzar", True, (255, 255, 255))
        sombra_enter = fuente_parpadeo.render("Pulsa ENTER para comenzar", True, (0, 0, 0))
        rect_enter = texto_enter.get_rect(bottomright=(ANCHO - 30, int(ALTO * 0.95)))
        VENTANA.blit(sombra_enter, (rect_enter.x + 2, rect_enter.y + 2))
        VENTANA.blit(texto_enter, rect_enter)

    pygame.display.flip()

# Iniciar el nivel directamente tras el diálogo
nivel_iniciado = True
tiempo_inicio = pygame.time.get_ticks()
...
while juego_activo:
    ...
    for evento in pygame.event.get():
            ...
            if evento.key in (pygame.K_UP, pygame.K_w):
                ...
                if indice_menu_pausa == 1:
                    ...
                    if music_volume > 0:
                        music_muted = False
                    aplicar_volumen_musica()
                elif indice_menu_pausa == 2:
                    sfx_volume = max(0.0, round(sfx_volume - 0.1, 2))
                    if sfx_volume > 0:
                        sfx_muted = False
                    aplicar_volumen_efectos()
                ...
                if indice_menu_pausa == 1:
                    ...
                    sfx_volume = min(1.0, round(sfx_volume + 0.1, 2))
                    if sfx_volume > 0:
                        sfx_muted = False
                    aplicar_volumen_efectos()
                elif indice_menu_pausa == 3:
        ...
        if evento.type == pygame.KEYDOWN and evento.key == pygame.K_r and (game_over or nivel_completado):
            # Si el nivel se completó, salir con éxito en lugar de reiniciar
            if nivel_completado:
                print("¡Nivel 2 completado exitosamente!")
                pygame.quit()
                sys.exit(0)  # Código 0 indica éxito
            else:
                # Si es game over, permitir reiniciar
                reiniciar_partida()
        if evento.type == pygame.KEYDOWN and evento.key == pygame.K_RETURN and nivel_completado:
            print("¡Nivel 2 completado exitosamente!")
            pygame.quit()
            sys.exit(0)
        if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1 and nivel_completado and boton_nivel3_rect:
            if boton_nivel3_rect.collidepoint(evento.pos):
                print("¡Nivel 2 completado exitosamente!")
                pygame.quit()
                sys.exit(0)
        if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1 and game_over and boton_reiniciar_rect:
            if boton_reiniciar_rect.collidepoint(evento.pos):
                reiniciar_partida()
    ...
    if nivel_iniciado and tiempo_transcurrido >= duracion_nivel and not game_over and not nivel_completado:
        ...
        # Establecer tiempo de nivel completado para el temporizador automático
    ...
    if menu_pausa_activo and not game_over and not nivel_completado:
        ...
        opciones = [
            "Continuar",
            f"Vol. Música: {'[OFF]' if music_muted else f'{int(music_volume * 100)}%'}",
            f"Vol. Efectos: {'[OFF]' if sfx_muted else f'{int(sfx_volume * 100)}%'}",
    ...
    # Si estás en game over, mostrar imagen y botón de reinicio
    if game_over:
        if not gameover_bg_cargada:
            gameover_bg_cargada = True
            try:
                img_path = os.path.join(os.path.dirname(__file__), 'imagenes', 'gameover2.png')
                if os.path.exists(img_path):
                    gameover_bg = pygame.image.load(img_path).convert()
            except Exception as e:
                print(f"Error cargando gameover2.png: {e}")

        if gameover_bg:
            bg_scaled = pygame.transform.scale(gameover_bg, (ancho_ui, alto_ui))
            VENTANA.blit(bg_scaled, (0, 0))
        else:
            overlay_go = pygame.Surface((ancho_ui, alto_ui), pygame.SRCALPHA)
            overlay_go.fill((0, 0, 0, 160))
            VENTANA.blit(overlay_go, (0, 0))
            fuente_big = pygame.font.SysFont("Arial", 72)
            texto_perdiste = fuente_big.render("HAS PERDIDO", True, (200, 0, 0))
            VENTANA.blit(texto_perdiste, texto_perdiste.get_rect(center=(ancho_ui // 2, alto_ui // 2 - 40)))

        btn_w = 220
        btn_h = 50
        btn_x = (ancho_ui - btn_w) // 2
        btn_y = (alto_ui // 2) + 180
        boton_reiniciar_rect = pygame.Rect(btn_x, btn_y, btn_w, btn_h)
        pygame.draw.rect(VENTANA, (70, 130, 180), boton_reiniciar_rect)
        fuente_btn = pygame.font.Font(None, 30)
        texto_btn = fuente_btn.render("Reiniciar (R)", True, (255, 255, 255))
        VENTANA.blit(texto_btn, texto_btn.get_rect(center=boton_reiniciar_rect.center))
    
    # Mostrar mensaje de nivel completado
    if nivel_completado:
        if not victoria_bg_cargada:
            victoria_bg_cargada = True
            try:
                img_path = os.path.join(os.path.dirname(__file__), 'imagenes', 'nivel2completado.png')
                if os.path.exists(img_path):
                    victoria_bg = pygame.image.load(img_path).convert()
            except Exception as e:
                print(f"Error cargando nivel2completado.png: {e}")

        if victoria_bg:
            bg_scaled = pygame.transform.scale(victoria_bg, (ancho_ui, alto_ui))
            VENTANA.blit(bg_scaled, (0, 0))
        else:
            ...
            VENTANA.blit(texto_completado, texto_completado.get_rect(center=(ancho_ui // 2, alto_ui // 2 - 40)))

        # Botón Ir al Nivel 3
        btn_w = min(320, ancho_ui // 3)
        btn_h = min(64, alto_ui // 10)
        btn_x = (ancho_ui - btn_w) // 2
        btn_y = int(alto_ui * 0.82)
        boton_nivel3_rect = pygame.Rect(btn_x, btn_y, btn_w, btn_h)
        mouse_pos = pygame.mouse.get_pos()
        hover = boton_nivel3_rect.collidepoint(mouse_pos)
        color_btn = (255, 200, 0) if hover else (70, 130, 180)
        pygame.draw.rect(VENTANA, color_btn, boton_nivel3_rect, border_radius=10)
        fuente_btn = pygame.font.SysFont("Arial", max(22, btn_h // 2), bold=True)
        texto_btn = fuente_btn.render("Ir al Nivel 3", True, (0, 0, 0) if hover else (255, 255, 255))
        VENTANA.blit(texto_btn, texto_btn.get_rect(center=boton_nivel3_rect.center))
...
pygame.quit()
sys.exit(1)  # Si sale del bucle por otra razón, se considera abandono